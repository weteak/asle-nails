...
class AppointmentService
{
    ...
    public function getAvailableHours() {
        ...
        for ($hour = $startHour; $hour < $endHour; $hour++) {
            // Check the number of appointments for each hour
            $appointmentsCount = Appointments::whereBetween('appointment_time', [
                Carbon::today()->setHour($hour),
                C\arbon::today()->setHour($hour + 1)
            ])->count();

            // If the number of appointments is less than the number of staff, the hour is available
