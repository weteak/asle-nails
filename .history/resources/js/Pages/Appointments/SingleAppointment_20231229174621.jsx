...
const formatTime = (time) => {
    ...
    return new Date(`1970-01-01T${time}`).toLocaleTimeString('de-DE', options);
     ...
     return <div>
        ...
        <div className="w-full border-y py-4 mt-2">
        <img className="w-32 my-auto object-cover rounded-full mt-5 h-32" src={returnImage(imagePathFormat(appointment.staff.profile_image))} alt={`${appointment.staff.profile_image}`}/>
        </div>
        </div>
     </div>
}