...
export default function SingleAppointment({ appointment , user }) {
    ...
    function convertDateFormat(dateString) {
        ...
        // Options for the toLocaleDateString function
        const options = {
            weekday: 'short', 
            year: 'numeric', 
            month: 'short', 
        ...
        formattedDate += ` ${date.toTimeString().split(' ')[0]} GMT${date.getTimezoneOffset() / -60 >= 0 ? '+' : ''}${Math.abs(date.getTimezoneOffset() / 60).toString().padStart(2, '0')}00`;
    
        // If you need to append a specific timezone name manually
        // formattedDate += ' (Central European Standard Time)';
    ...
    return <div>
        ...
        {convertDateFormat(appointment.appointment_date)}

        {openEdit ? <ChooseAppointments edit={true} selTime={formatTime(appointment.appointment_time)} user={user} serviceSelected={appointment.service.id} staffSelected={appointment.staff}/> :
        <div className="mt-4">
