...
export default function SingleAppointment({ auth, appointment }) {
    ...
    function callPhoneNumber(phoneNumber) {
        // This will attempt to open the phone dialer with the number pre-filled
    ...
    return <div>
        ...
        <div className="mt-4">
            ...
            <div className="w-full flex gap-2 relative border-y py-4 mt-2">
                <div className=" px-7 flex flex-col gap-1 text-left">
                    <h2 className="text-black text-lg font-semibold">{appointment.staff.name}</h2>
                    <span className="px-2 w-fit bg-green-500 text-green-800 font-semibold bg-opacity-5 py-1.5">+355{appointment.staff.phone_number}</span>
                    <div className="mt-4 flex gap-2 "> Call it<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" className="bi my-auto bi-telephone" viewBox="0 0 16 16">
                ...
                <img className="w-24 absolute top-1/2 -translate-y-1/2 right-2 mx-2 my-auto object-cover rounded-full  h-24" src={returnImage(imagePathFormat(appointment.staff.profile_image))} alt={`${appointment.staff.profile_image}`} />

            </div>
            <div className="mx-5 grid grid-cols-1 gap-2 mt-4">
                <button className="w-full bg-opacity-85 bg-blue-600 py-3.5 rounded-lg text-white">Edit Appointment</button>
                <button className="w-full bg-opacity-10 text-red-600 bg-red-600 py-3.5 rounded-lg ">Cancel Appointment</button>
