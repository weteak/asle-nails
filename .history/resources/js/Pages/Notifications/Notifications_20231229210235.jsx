...
import { extractLinkSuffix } from "@/Utils/helpers";


const RedirectLink = ({ message, children }) => {
    const filteredMessage = extractLinkSuffix(message);
    return filteredMessage ? <Link href={filteredMessage}>{children}</Link> : children;
};
    ...
    return <div>
        ...
        <div className="flex bg-opacity-10  w-full flex-col gap-0">
            {hasNotifications && notifications.map((val, key) => {
                return <RedirectLink message={val.message}><motion.div initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                 ...
                 bg-opacity-10 relative border-slate-400 border-b border-opacity-15  flex gap-2.5`}>
                    ...
                    <span className="font-medium">{val.message.split('link')[0]} <span style={{ fontSize: '11px' }} className=" text-slate-600 ml-0.5 rounded-sm bg-opacity-90  border-slate-500 ">{convertDateFormat(val.created_at)}</span></span>
                </motion.div></RedirectLink>
            })}
        </div>
    </div>
}