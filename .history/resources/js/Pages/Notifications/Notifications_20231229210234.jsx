            ...
            {hasNotifications && notifications.map((val, key) => {
                return <RedirectLink message={val.message}><motion.div initial={{ opacity: 0 }}
                 ...
                 bg-opacity-10 relative border-slate-400 border-b border-opacity-15  flex gap-2.5`}>
                    ...
                    <span className="font-medium">{val.message.split('link')[]} <span style={{ fontSize: '11px' }} className=" text-slate-600 ml-0.5 rounded-sm bg-opacity-90  border-slate-500 ">{convertDateFormat(val.created_at)}</span></span>
