...
export default function FriendsWidget({ data, setTransferOpen, setChoosenUser }) {
    ...
    return <div className='m-3  '>
        ...
        <div className='flex mt-3 border border-slate-500 border-opacity-50   shadow-md rounded-lg flex-col'>
            {data.friends.length > 0 && data.friends.map((val,key) => {
                return <div key={`friend-${key}`} className={`p-3 ${key !== data.friends.length-1 && 'border-b'} relative border-b`}>
