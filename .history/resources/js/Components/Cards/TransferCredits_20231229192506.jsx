                    ...
                    <div className='grid mt-2 gap-1 m-3 items-center grid-cols-2'>
                        {packetsToShare.filter((val) => val <= info.asle_card.points).map((packet, key) => (
                            <div
                                ...
                                key={key}
                                className={`p-3 rounded-md ${packet === data.credit ? 'border border-dashed  border-blue-500' : 'border-transparent'} border whitespace-nowrap ${key === 1 ? 'bg-green-500' : key === 2 ? 'bg-violet-500' : key === 3 ? 'bg-red-500 ' : 'bg-blue-500'
                                    } bg-opacity-10`}
                            >
