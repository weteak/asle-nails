    ...
    return openTransfer !== null && (
        <>
            ...
            <motion.form onSubmit={transfer} initial={{ opacity: 0, y: '4%' }}
                ...
                exit={{ opacity: 0, y: '4%' }}
                transition={{ duration: 0.3, ease: 'easeInOut' }} className='fixed p-2 left-4 border border-slate-500 border-opacity-50 shadow-xl rounded-xl right-4 top-[20%] -translate-y-1/2 z-40 bg-white'>
                    ...
                    <div className='grid mt-2 gap-1 m-3 items-center grid-cols-2'>
                        {packetsToShare.filter((val) => val <= info.asle_card.points).map((packet, key) => (
                            <div
                                ...
                                key={key}
                                className={`p-3 rounded-md ${packet === data.credit ? 'border-2 border-dashed  border-blue-500' : 'border-transparent'} border whitespace-nowrap ${key === 1 ? 'bg-green-500' : key === 2 ? 'bg-violet-500' : key === 3 ? 'bg-red-500 ' : 'bg-blue-500'
                                    } bg-opacity-10`}
                            >
                                {data.credit}
