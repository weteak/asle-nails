...
export default function UsePoints({ showModal, handleUsePoints, points, points_value }) {
    return showModal && (
        <div className="fixed inset-0 bg-blue-500 backdrop-blur-md bg-opacity-5 overflow-y-auto h-full w-full">
            <motion.div initial={{ opacity: 0, y: '4%' }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: '4%' }}
                transition={{ duration: 0.25, ease: 'easeInOut' }} className="relative top-1/4 mx-auto p-5 border border-slate-500 border-opacity-40 w-80 shadow-lg rounded-md bg-white">
                ...
                <div className="mt-3 text-center">
                    ...
                    <div className="items-center px-4 py-3">
                        ...
                        <button id="cancel-btn" className="px-4 py-2 mt-3 underline bg-white text-indigo-500 text-base font-medium rounded-md w-full  hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-900" onClick={()=>hand}>
                            No, maybe later
                        </button>
                    </div>
                </div>
