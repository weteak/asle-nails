    ...
    return showModal && (<>
        <div ref={element} onClick={() => setShowModal(false)} className="fixed inset-0 bg-blue-500 backdrop-blur-md bg-opacity-5 overflow-y-auto h-full w-full"></div>
            <motion.div initial={{ opacity: 0, y: '4%' }}
                ...
                exit={{ opacity: 0, y: '4%' }}
                transition={{ duration: 0.25, ease: 'easeInOut' }} className="fixed  left-4 right-8 top-1/4  p-5 border border-slate-500 border-opacity-40  shadow-lg rounded-md bg-white">
