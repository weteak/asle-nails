...
export default function UsePoints({ showModal, setShowModal, handleUsePoints, points, points_value }) {
    return showModal && (<>
        <div onClick={() => setShowModal(false)} className="fixed inset-0 bg-blue-500 backdrop-blur-md bg-opacity-5 overflow-y-auto h-full w-full"></div>
        <motion.div initial={{ opacity: 0, y: '4%' }}
            ...
            <div className="mt-3 text-center">
                ...
                <div className="items-center px-4 py-3">
                    <button id="ok-btn" className="px-4 py-2 bg-indigo-500 text-white text-base font-medium rounded-md w-full shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-900" onClick={() => handleUsePoints(true)}>
                        Yes, use my points
                    </button>
                    <button id="cancel-btn" className="px-4 py-2 mt-3 underline bg-white text-indigo-500 text-base font-medium rounded-md w-full  hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-900" onClick={() => handleUsePoints(false)}>
                        No, maybe later
                    </button>
                </div>
            </div>
