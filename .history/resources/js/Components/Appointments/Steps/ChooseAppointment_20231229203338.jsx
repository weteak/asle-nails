...
export default function ChooseAppointments({
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (user.asle_card && user.asle_card.points >= 50 && !edit) {
            setShowModal(true);
        } else {
            bookAppointment();
        }
    ...
    const bookAppointment = (usePoints = false) => {
        const basicAppointmentData = createAppointmentData(serviceSelected.id, selectedTime, staffSelected.id, usePoints);
