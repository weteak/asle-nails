...
export default function ChooseAppointments({ user, serviceSelected, staffSelected }) {
    ...
    console.log(selectedDate.toLocaleDateString('en-AL'));
    
    const formated = selectedDate.toLocaleDateString('en-AL').split('/');
    console.log(`${formated[formated.length-1]}-${formated[1]}-${formated[0]}`);

    const bookAppointment = (usePoints = false) => {
        const appointmentData = {
            service_id: serviceSelected.id,
            appointment_date: selectedDate.toLocaleDateString('en-AL').split('T')[0],
    ...
    return <> <form onSubmit={handleSubmit} className="space-y-4 mt-6">
        <div>
            ...
            <div className='sticky top-16 pt-4 w-full bg-white py-2 px-1 flex gap-1 '>
                <span>{serviceSelected.service_name}  <span className='ml-2 bg-blue-500 bg-opacity-5  py-1 font-bold rounded-lg text-blue-600'>{selectedDate.toLocaleString('default', { day: 'numeric' }) + ' ' + selectedDate.toLocaleString('default', { month: 'short' })}  {selectedTime && `- ${selectedTime}`}</span>   <span className='ml-2'>me {staffSelected.name}.</span> </span>
            </div>
        ...
        <button type="submit" className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Book Appointment</button>
    </form>
