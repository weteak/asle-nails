            ...
            {services.map((val, key) => {
                return <div key={`service-${key}`} className="w-full border-b py-4 border-opacity-10  relative
                 border-slate-600  flex gap-2  ">
                    <img className="w-14 rounded-full h-14 object-cover object-center" src={returnImage(imagePathFormat(val.image))} />
                    ...
                    <span onClick={() => { setStep(1); setServiceSelected(val) }} className="absolute top-1/2 -translate-y-1/2 right-4">
