...
export default function Steps({ services, user, staff }) {
    ...
    const steps = [<ChooseService
        ...
        , <ChooseAppointments user={user} serviceSelected={serviceSelected} />];
    ...
    return <div>
        ...
        <div className='h-1 mt-4 w-full bg-gray-100 rounded-sm'>
            <div className={` ${currentStep == 0 ? 'w-0' : currentStep == 1 ? 'w-[33%]' : 'w-1/2'}  h-1 rounded-l-sm relative bg-blue-600 bg-opacity-70`}>
            </div>
        </div>
