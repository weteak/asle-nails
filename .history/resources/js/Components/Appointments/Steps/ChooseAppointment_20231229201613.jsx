...
export default function ChooseAppointments({
    ...
    const formatAppointmentDate = (dateArray) => {
        return `${dateArray[dateArray.length - 1]}-${dateArray[1]}-${dateArray[0]}`;
    };

    const createAppointmentData = (serviceId, formattedDate, time, staffId, usePoints) => {
        return {
            service_id: serviceId,
            appointment_date: formatAppointmentDate(formattedDate),
    ...
    const bookAppointment = (usePoints = false) => {
        const formattedDate = formatAppointmentDate(formatedDate);
        const basicAppointmentData = createAppointmentData(serviceSelected.id, formattedDate, selectedTime, staffSelected.id, usePoints);
        const url = edit ? '/update-appointment' : '/create-appointment';
        const data = edit ? { ...basicAppointmentData, appointment_id: appointment.id, use_points: appointment.use_points } : basicAppointmentData;
