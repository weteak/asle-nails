    ...
    return <> <form onSubmit={handleSubmit} className="space-y-4 mt-6">
        <div>
            ...
            <div className='sticky top-16 pt-4 w-full bg-white p-2 flex gap-1 '>
                <span>{serviceSelected.service_name}  <span className='ml-2 bg-blue-500 bg-opacity-5 px-2 py-1 font-bold rounded-lg text-blue-600'>{selectedDate.toLocaleString('default', { day: 'numeric' }) + ' ' + selectedDate.toLocaleString('default', { month: 'short' })}  {selectedTime && `- ${selectedTime}`}</span>   <span>me {staffSelected.name}</span> </span>
            </div>
