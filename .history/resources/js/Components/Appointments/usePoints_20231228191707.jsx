    ...
    return showModal && (<>
        <div ref={element} onClick={() => setShowModal(false)} className="fixed inset-0 bg-blue-500 backdrop-blur-md bg-opacity-5 overflow-y-auto h-full w-full"></div>
            <motion.div initial={{ opacity: 0, y: '4%' }}
                ...
                exit={{ opacity: 0, y: '4%' }}
                transition={{ duration: 0.25, ease: 'easeInOut' }} className="fixed -translate-x-1/2 left-1/4 top-1/4 mx-auto p-5 border border-slate-500 border-opacity-40 w-80 shadow-lg rounded-md bg-white">
