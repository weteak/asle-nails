...
export default function Authenticated({ user, header, children }) {
    ...
    useScrollLock(showingNavigationDropdown);
                    ...
                    <div className="flex justify-between h-16">
                        ...
                        <AnimatePresence>
                            {showingNavigationDropdown && <>
                                <div onClick={()=>setShowingNavigationDropdown(false)} className='fixed bg-white backdrop-blur-sm bg-opacity-50 z-50 w-full top-0 h-screen left-0'></div>
                                <motion.div initial={{ opacity: 0, x: '20%' }}
                                    animate={{ opacity: 1, x: 0 }}
                                    className='h-fit -translate-x-1/2 top-1/2 left-1/4 absolute w-fit  flex flex-col shadow-md   z-50  border-l right-0 border-slate-400 border-opacity-20 bg-white'>
