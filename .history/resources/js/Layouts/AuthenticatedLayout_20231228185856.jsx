                            ...
                            <div className={`pt-2 pb-2  w-full md:hidden  items-center border-b border-slate-500
                             border-opacity-20 ${grid}`}>
                                {dataMobile.map((val, key) => {
                                    return <div key={`menu-${key}`} className={`${route().current(val.route) && 'bg-blue-600 bg-opacity-10 text-blue-600'} hover:bg-blue-500 hover:bg-opacity-10 hover:text-blue-500 animation-all duration-200 mx-auto rounded-full p-3 my-auto w-fit `}>
                                ...
                                {user.role === 'user' &&
                                    <div className={`relative hover:bg-blue-500 hover:bg-opacity-10 hover:text-blue-500 animation-all duration-200  mx-auto rounded-full p-3 w-fit `}>
                                        {user.notification_unread > 0 && <span className='absolute  p-1 right-1 z-40 bg-white top-0 font-bold text-indigo-600  rounded-full'>+{user.notification_unread}</span>
                                        }
                                        ...
                                        {user.notification_unread > 0 && <span className='absolute w-2 h-2 top-2 rounded-full bg-indigo-500 animate-pulse right-2'></span>}
                                    </div>}

                                <div onClick={() => setShowingNavigationDropdown(!showingNavigationDropdown)} className={`relative  mx-auto rounded-full p-3 w-fit `}>
                                    <div className=''>
